...
def generate_email_digest(hours: int = 24, top_n: int = 10) -> EmailDigestResponse:
    ...
    try:
        ...
        logger.warning(f"Ranking failed ({e}), will use articles in original order instead")
        # Fallback: use digests in original order if ranking fails
        ranked_articles = [
            type('RankedArticle', (), {
                'digest_id': d['id'],
                'rank': i + 1,
                'relevance_score': 5.0 + (total - i) * 0.1,  # Descending scores
                'reasoning': 'Used as fallback due to ranking service unavailable'
            })()
            for i, d in enumerate(digests[:top_n])
        ]
    
    if not ranked_articles:
        logger.error("Failed to rank digests and no fallback available")
